...
void focusdown() {
  ...
  if ((c->y + c->h + selmon->gappoh) == selmon->wh)
    return;
  ...
  for (iter = selmon->clients; iter != NULL; iter = iter->next) {
    ...
    if (!ISVISIBLE(iter))
      continue;
    if (iter->y == targetY || iter->y == (targetY + selmon->gappih) ||
      ...
      if ((devX < bestDevX && devY < bestDevY) ||
          (devX == bestDevX && devY < bestDevY) ||
          (devX < bestDevX && devY == bestDevY)) {
        bestDevX = devX;
        bestDevY = devY;
        bestCandidate = iter;
...
void focusup() {
  ...
  if ((c->y + selmon->gappoh) == selmon->wy)
    return;
  ...
  for (iter = selmon->clients; iter != NULL; iter = iter->next) {
    ...
    if (iterHY == targetY || iterHY == (targetY - selmon->gappih) ||
...
void focusleft() {
  ...
  for (iter = selmon->clients; iter != NULL; iter = iter->next) {
    ...
    if (iterWX == targetX || (iterWX == (targetX - selmon->gappiv)) ||
...
void focusright() {
  ...
  if ((c->x + c->w + selmon->gappov) == selmon->ww)
    return;
  ...
  for (iter = selmon->clients; iter != NULL; iter = iter->next) {
    ...
    if (iter->x == targetX || (iter->x == (targetX + selmon->gappiv)) ||
